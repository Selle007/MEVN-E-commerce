    ...
    <div class="mx-auto max-w-screen-xl px-4 lg:px-12">
      ...
      <div class="bg-white relative shadow-md sm:rounded-lg overflow-hidden">
        ...
        <div class="overflow-x-auto">
          <table class="w-full text-sm text-left text-gray-500" v-if="Messages.length > 0">
            ...
            <tbody>
              <tr class="border-b" v-for="msg in Messages" :key="msg._id">
                ...
                <td class="px-4 py-3 flex flex-row ">
                  <span class="inline-flex divide-x overflow-hidden rounded-md border bg-white shadow-sm">
                    <button class="inline-block p-3 text-gray-700 hover:text-emerald-600 hover:bg-gray-50 focus:relative"
                      title="Edit Product" @click="detailMessage(msg._id)">
                    ...
                    <button class="inline-block p-3 text-gray-700 hover:bg-gray-50 hover:text-red-600 focus:relative"
                      title="Delete Product" @click.prevent="deleteMessage(msg._id)">
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                        stroke="currentColor" class="h-4 w-4">
                        <path stroke-linecap="round" stroke-linejoin="round"
...
export default {
  ...
  created() {
    ...
    axios
      ...
      .then((res) => {
        ...
        //console.log(this.Messages); // Debugging statement
      })
      .catch((error) => {
        console.log(error);
      });
  ...
  methods: {
    deleteMessage(id) {
      let apiURL = `http://localhost:3000/api/delete-contact/${id}`;
      let indexOfArrayItem = this.Messages.findIndex((i) => i._id === id);

      if (window.confirm("Do you really want to delete?")) {
        axios
          .delete(apiURL)
          .then(() => {
            this.Messages.splice(indexOfArrayItem, 1);
          })
          .catch((error) => {
            console.log(error);
          });
    ...
    detailMessage: function (id) {
      this.$router.push(`/dashboard/messages/message-details/${id}`);
    },
  },
};
</script>
